...
Первый обязательный практический шаг ROOT-части после read-only аудита — настроить пользовательский SSH-ключ и удобный Windows SSH alias для `root`, чтобы дальше не вводить IP и пароль вручную. Парольную авторизацию на сервере этот гайд не отключает.
...
## 0.4. Пользовательский SSH-ключ компьютера для root и dev

Для обычного входа на сервер используется **пользовательский SSH-ключ Windows-компьютера**. Один и тот же подходящий пользовательский ключ можно использовать сначала для `root`, а после создания `dev` — и для `dev`. Это не GitHub deploy key.

Сначала Codex помогает проверить, есть ли на Windows уже подходящий публичный ключ. Нормальны как современные Ed25519-ключи, так и существующие корректные RSA-ключи. Не нужно генерировать новый ключ только потому, что старый не Ed25519.
...
Публичную часть существующего или нового ключа сначала добавляют в `/root/.ssh/authorized_keys`, а позже — в `/home/dev/.ssh/authorized_keys`. Приватный пользовательский ключ на сервер не копируется.

На Windows для сервера создаются SSH aliases `<name>-root` и `<name>-dev`, чтобы дальше подключаться короткими командами вроде `ssh gt-root` и `ssh gt-dev` без ручного ввода IP и пароля.

Эта настройка делается **для удобства**, а не как hardening: гайд не меняет `PasswordAuthentication`, `PermitRootLogin` и другие параметры `sshd_config`. Существующий парольный вход остаётся доступным как раньше.
...
4. публичный SSH-ключ пользователя, если подходящий ключ ещё не настроен для root/dev;
...
| 2. SSH-доступ root | root + пользователь | Пользовательский ключ добавлен root; Windows alias `<name>-root` реально входит по ключу без ввода пароля |
| 3. SSH-доступ dev | root + пользователь | После создания `dev` тот же пользовательский ключ и alias `<name>-dev` реально работают |
| 4. Права | root | `dev` пишет в `web/`, не пишет выше, owners/groups не сломаны |
| 5. Локальный checkpoint | root | Создан/подтверждён checkpoint всего проекта |
| 6. Root automation | root | Root checkpoint и cron проверены вручную |
| 7. Codex для dev | root | Рабочий bundled Codex SDK из root-cache заранее скопирован в cache `dev` и запускается от `dev` |
| 8. Общая Codex-среда | root | Superpowers и глобальный AGENTS подключены к root/dev; автосинхронизация AGENTS с GitHub установлена и проверена |
| 9. Handoff | root + пользователь | State сохранён; root Remote SSH полностью закрыт |

### Часть II — DEV

| Этап | Кто работает | Результат, без которого нельзя идти дальше |
|---|---|---|
| 10. Переподключение | пользователь | VS Code действительно работает как `dev`; ChatGPT login выполнен для `dev` |
| 11. GitHub deploy key | dev + пользователь | `ssh -T` подтверждает доступ к одной клиентской репе |
| 12. GitHub-контур | Codex как dev | Отдельная Git metadata и wrapper работают |
| 13. Данные и HTTP | Codex как dev | SQLite snapshot целый, закрытые URL дают 403/404 |
| 14. Dev automation | dev | Ручной GitHub-sync и первый push успешны |
| 15. Финальная проверка | Codex + пользователь | Все тесты пройдены, создан report, временный гайд удалён |
...
- первым практическим действием настраивает пользовательский SSH-ключ для `root`, Windows alias `<name>-root` и проверяет реальный вход по ключу; парольную авторизацию при этом не отключает и `sshd_config` ради этого не меняет;
- создаёт/проверяет `dev` без sudo;
- добавляет тот же пользовательский SSH-ключ для `dev`, создаёт/проверяет alias `<name>-dev`;
...
- Windows alias `<name>-root` ранее был реально проверен входом по пользовательскому ключу без ввода пароля;
- Windows alias `<name>-dev` реально входит по пользовательскому ключу;
...
## ROOT-1. SSH-ключ и Windows alias для root — первый обязательный шаг

Это первое практическое изменение ROOT-части. Цель — чтобы после начального входа по паролю пользователь дальше подключался к серверу короткой командой вроде `ssh gt-root`, без ручного ввода IP и пароля.

**Парольную авторизацию не отключать.** Не менять ради этого `PasswordAuthentication`, `PermitRootLogin` или другие параметры `sshd_config`. Ключ добавляется как удобный дополнительный способ входа.

### 1. Проверить пользовательский SSH-ключ на Windows
...
Если уже есть подходящий пользовательский ключ, использовать его. Если ключей нет, создать Ed25519:

```powershell
ssh-keygen -t ed25519
```

Показать публичную часть выбранного ключа, например:

```powershell
Get-Content "$env:USERPROFILE\.ssh\id_ed25519.pub"
```

Публичный ключ не является секретом. Приватный файл без `.pub` в чат не отправлять и на сервер не копировать.

### 2. Добавить публичный ключ root

В уже открытой root-сессии на сервере:
...
Затем Codex добавляет **точную публичную строку**, которую дал пользователь, в `/root/.ssh/authorized_keys`, только если её там ещё нет. Существующие ключи не удалять и файл целиком не перезаписывать.

### 3. Создать удобный alias на Windows

В `C:\Users\<USER>\.ssh\config` добавить или аккуратно объединить блок:
...
`<name>` — короткое удобное имя сервера, например `gt`. Если используется другой существующий приватный ключ, в `IdentityFile` указывается его фактический путь.

Существующий `C:\Users\<USER>\.ssh\config` не перезаписывать. Если для этого же alias позже нужны `RemoteForward`, `LocalForward` или другие параметры из отдельного VPN/Codex-гайда, они добавляются в тот же `Host`, а не создаётся второй конфликтующий блок.

### 4. Обязательно проверить новый вход

В **новом** Windows PowerShell сначала проверить именно key-only вход:

```powershell
ssh -o PreferredAuthentications=publickey -o PasswordAuthentication=no <name>-root "echo KEY_OK"
```
...
должна подключать к серверу без запроса root-пароля. Только после этой практической проверки ROOT-маршрут продолжается дальше.
...
Тот же пользовательский публичный SSH-ключ, который был проверен для `root`, добавляется в:
...
Существующие ключи `dev` не удалять; публичную строку добавлять только при её отсутствии.

На Windows в тот же `C:\Users\<USER>\.ssh\config` добавить/объединить alias для `dev`:
...
Если фактический пользовательский ключ другой, использовать его реальный путь. Существующий SSH config не перезаписывать.

До изменения production-прав пользователь практически проверяет вход под `dev` из нового Windows PowerShell:

```powershell
ssh -o PreferredAuthentications=publickey -o PasswordAuthentication=no <name>-dev "echo KEY_OK"
```

После этого обычный `ssh <name>-dev` также должен входить без пароля. Если вход не работает, дальнейшая настройка прав не выполняется. После тестового входа dev-сессию закрыть и продолжить ROOT-часть под `root`.
...
- [ ] Пользовательский SSH-ключ добавлен для `root`; Windows alias `<name>-root` проверен key-only входом и обычным входом без запроса пароля.

- [ ] Парольная SSH-авторизация не отключалась и `sshd_config` ради convenience-настройки ключа не менялся.

- [ ] Создан `dev` без sudo.

- [ ] Тот же пользовательский SSH-ключ добавлен для `dev`; Windows alias `<name>-dev` проверен key-only входом.
