...
- If the current task becomes materially context-heavy, repeated compaction would be needed, or a completed phase has a clean handoff boundary, propose a short handoff summary/file and a fresh thread instead of indefinitely growing the same session. Do not create handoff ceremony for routine short tasks.
